...
Aws::UniquePtr<TelemetryProvider> OtelTelemetryProvider::CreateOtelProvider() {
    ...
    return Aws::MakeUnique<TelemetryProvider>(ALLOC_TAG, std::move(tracerProvider),
        ...
        [&]() -> void {
            //Init Tracing
            auto traceExporter = opentelemetry::exporter::trace::OStreamSpanExporterFactory::Create();
            auto traceProcessor = opentelemetry::sdk::trace::SimpleSpanProcessorFactory::Create(std::move(traceExporter));
            std::vector<std::unique_ptr<opentelemetry::sdk::trace::SpanProcessor>> traceProcessors;
            traceProcessors.push_back(std::move(traceProcessor));
            std::shared_ptr<opentelemetry::sdk::trace::TracerContext> context = opentelemetry::sdk::trace::TracerContextFactory::Create(std::move(traceProcessors));
            std::shared_ptr<opentelemetry::trace::TracerProvider> otelTracerProvider = opentelemetry::sdk::trace::TracerProviderFactory::Create(context);
            opentelemetry::trace::Provider::SetTracerProvider(otelTracerProvider);
            //Init Metrics
            std::unique_ptr<opentelemetry::sdk::metrics::PushMetricExporter> metricsExporter{new opentelemetry::exporter::metrics::OStreamMetricExporter};
            opentelemetry::sdk::metrics::PeriodicExportingMetricReaderOptions options;
            options.export_interval_millis = std::chrono::milliseconds(1000);
            options.export_timeout_millis = std::chrono::milliseconds(500);
            std::unique_ptr<opentelemetry::sdk::metrics::MetricReader> reader{new opentelemetry::sdk::metrics::PeriodicExportingMetricReader(std::move(metricsExporter), options)};
